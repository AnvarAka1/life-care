...
Config.init_emoticons = function () {
  ...
  for (const i in Config.emoticons_data) {
    // because we never see some characters in our text except as
    // entities, we must do some replacing
    const emoticon = i.replace(/\&/g, '&amp;').replace(/\</g, '&lt;')
      .replace(/\>/g, '&gt;')
...
Config.init_colons = function () {
  ...
  for (const i in Config.data) {
    for (let j = 0; j < Config.data[i][3].length; j++) {
...
function buildMap () {
  const colons = []; const codes = []
  for (const i in Config.emoji_data) {
    for (let j = 0; j < Config.emoji_data[i][0].length; j++) {
